...
import (
	...
	"github.com/mdfriday/themes-builder/pkg/client"
	"github.com/peterbourgon/ff/v3/ffcli"
	"github.com/rogpeppe/go-internal/semver"

	"github.com/mdfriday/themes-builder/pkg/rootcmd"
)
...
func (c *Config) Exec(ctx context.Context, args []string) error {
	...
	fmt.Println("after create themes config")

	var err error
	bc.mmap, err = bc.GetHugoModulesMap(configAll)
	if err != nil {
		fmt.Printf("failed to get Hugo modules map: %s\n", err)

		return err
	}

	fmt.Println("before init gh repos")

	bc.initGhRepos(c.cleanCache)

	fmt.Println("after init gh repos")
	...
	fmt.Println("before write themes content")

	if err := bc.writeThemesContent(); err != nil {
		return err
	}

	fmt.Println("after write themes content")
