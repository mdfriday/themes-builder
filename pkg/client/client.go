...
const (
	modPath  = "github.com/mdfriday/themes-builder/cmd/themesitebuilder/build"
	cacheDir = "cache"
)
...
func (c *Client) GetHugoModulesMap(config string) (ModulesMap, error) {
	...
	fmt.Println("get hugo modules map")

	b := &bytes.Buffer{}
	if err := c.runHugo(b, "--config", config, "config", "mounts"); err != nil {
		fmt.Println("runHugo...", b.String())

		return nil, err
	}

	fmt.Println("got hugo modules map")
	...
	for k, v := range mmap {
		...
		if _, err := os.Stat(filename); err == nil {
			...
			fmt.Println("read theme.toml", filename, err)
...
func (c *Client) runHugo(w io.Writer, arg ...string) error {
	...
	fmt.Println("run hugo", arg, c.outDir)
...
func doGitHubRequest(req *http.Request, v interface{}) error {
	...
	fmt.Println("github response", req.URL, resp.Status)
